...
export class LangchainService implements OnModuleInit {
  ...
  onModuleInit() {
    this.llm = new ChatOpenAI({ openAIApiKey: process.env.OPENAI_API_KEY, modelName: 'gpt-4o' , temperature : 0.7});
  ...
  async runPrompt(filename : string, userInput : string){
    ...
    return await this.llm.invoke(finalPrompt)
  }
  ...
  async preInterviewEval(dto : PreEvalDto){
    ...
    const result = await this.llm.invoke(combinedPrompt.replace('User Question Input : ', `User Question Input : ${dto.question}`));
  ...
  async interview(
    ...
    if(isInit) {
      //Reset memory for new interview session
      this.resetMemory()
      ...
      //For follow-up questions, just send the question directly to the chain
      let initResponse = await this.chain.call({input : question})
      transcript.push({question : question, answer : initResponse.response})
      console.log("----Chat History---")
      console.log(this.memory.chatHistory)
      return transcript
  ...
  async postEval(qnas : Qna[], sessionId?: string, userId?: string, persona?: string){
    ...
    const result = await this.llm.invoke(evalPrompt);
